...
float DecodeArrayIndexFromColor(float4 color)
{
    return floor(color.r * 255.0 + 0.5);
}
...
void SetupDOTSLitMaterialPropertyCaches()
{
    ...
    unity_DOTS_Sampled_DetailAlbedoMapScale = UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(
float, _DetailAlbedoMapScale);
    unity_DOTS_Sampled_DetailNormalMapScale = UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(
...
half3 ApplyDetailNormal(float2 detailUv, half3 normalTS, half detailMask)
{
    ...
    #else
    half3 detailNormalTS = UnpackNormalScale(SAMPLE_TEXTURE2D(_DetailNormalMap, sampler_DetailNormalMap, detailUv),
...
inline void InitializeStandardLitSurfaceData(float2 uv, float arrayIndex, out SurfaceData outSurfaceData)
{
    half4 albedoAlpha = SAMPLE_TEXTURE2D_ARRAY(_BaseMapArray, sampler_BaseMapArray, uv, arrayIndex);
...
{
    InitializeStandardLitSurfaceData(uv, 0.0, outSurfaceData);
}
